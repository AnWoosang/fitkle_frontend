...
interface VersionInfo {
  ...
  filename: string;       // 20241217-location-terms-v1.0.md
  effectiveDate: string;  // 2024년 12월 17일
}

export default function LocationTermsArchivePage() {
  ...
  if (fs.existsSync(archiveDir)) {
    const files = fs.readdirSync(archiveDir)
      .filter(file => file.includes('location-terms') && file.endsWith('.md'))
      .sort()
      .reverse(); // 최신순 정렬

    versions = files.map(filename => {
      // 파일명에서 정보 추출: 20241217-location-terms-v1.0.md
      const match = filename.match(/^(\d{8})-location-terms-(v[\d.]+)\.md$/);
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <h1 className="text-3xl font-bold mb-8">위치기반서비스 이용약관 - 이전 버전</h1>
        ...
        {versions.length === 0 ? (
          ...
          <div className="space-y-4">
            {versions.map(({ version, effectiveDate }) => (
              ...
              >
                <Link href={`/legal/location-terms/archive/${version}`}>
